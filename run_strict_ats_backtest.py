...
def load_symbol_parquet_2m(symbol):
    ...
    df_all['group_15m'] = df_all['open_time'] // 900000
...
def backtest_parquet_symbol_strict(symbol, ats_threshold_15m=1.0, ats_threshold_1m=0.5, position_size_usdt=100.0, initial_capital_usdt=100.0):
    ...
    for i in range(start_idx_15m, len(records_15m)):
        ...
        # STRICT ATS FILTERING: Requires strong momentum
        if abs(ats_15m) < ats_threshold_15m:
            continue
            
        direction = "LONG" if ats_15m >= ats_threshold_15m else "SHORT"
        ...
        # STRICT 1M ATS FILTERING
        if direction == "LONG" and ats_1m < ats_threshold_1m:
            continue
        if direction == "SHORT" and ats_1m > -ats_threshold_1m:
            continue
        ...
        max_forward = min(len(records_1m), idx_1m + 1 + 2880)
...
def run_experiment(ats_15m_thresh, ats_1m_thresh):
    symbols = [d for d in os.listdir(PARQUET_DIR) if d.endswith('USDT') and os.path.isdir(os.path.join(PARQUET_DIR, d))]
    symbols.sort()

    print("==========================================================================================")
    print(f"🎯 YÜKSEK ATS FİLTRESİ BACKTESTİ | 15m ATS Eşiği: ≥ |±{ats_15m_thresh:.1f}| | 1m ATS Eşiği: ≥ |±{ats_1m_thresh:.1f}|")
    print("==========================================================================================")

    results = []
    
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = {executor.submit(backtest_parquet_symbol_strict, sym, ats_15m_thresh, ats_1m_thresh): sym for sym in symbols}
        count = 0
    ...
    if not results:
        print("❌ Sıkı filtreleme ile işlem üretilemedi!")
        return
    ...
    print("\n" + "=" * 115)
    print(f"📊 SIKI ATS FİLTRESİ (15m ATS ≥ |±{ats_15m_thresh:.1f}| & 1m ATS ≥ |±{ats_1m_thresh:.1f}|) TOPLU ÖZETİ")
    print("=" * 115)
    ...
    # Save to JSON & Excel
    report_data = {
        'ats_15m_threshold': ats_15m_thresh,
        'ats_1m_threshold': ats_1m_thresh,
    ...
    json_out = f"/home/smhvz/Desktop/cycle-orc/data/strict_ats_{ats_15m_thresh:.1f}_report.json"
    with open(json_out, "w") as f:
        json.dump(report_data, f, indent=2)

    # Export Excel summary table
    df_summary = pd.DataFrame([{
        'Sıra': idx,
        'Parite': r['symbol'],
        'Toplam İşlem Sayısı': r['total_trades'],
        'Kazanma Oranı (%)': round(r['win_rate'], 2),
        'Net Kâr / Zarar ($)': round(r['total_pnl_usdt'], 2),
        'Kasa Büyümesi (%)': round(r['total_pnl_usdt'], 2),
        'Bitiş Bakiyesi ($)': round(r['ending_equity'], 2)
    } for idx, r in enumerate(sorted_results, 1)])
    
    excel_out = f"/home/smhvz/Desktop/cycle-orc/strict_ats_{ats_15m_thresh:.1f}_summary.xlsx"
    df_summary.to_excel(excel_out, index=False)
    print(f"\n📂 ÖZET EXCEL RAPORU KAYDEDİLDİ: {excel_out}")

if __name__ == "__main__":
    run_experiment(ats_15m_thresh=1.5, ats_1m_thresh=0.5)
