        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_trades(symbols, is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
                                ...
                                if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                    ...
                                    if !symbol.is_empty() {
                                        let output = serde_json::json!({
                                            ...
                                            "price": json["p"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0),
                                            "qty": json["q"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0),
                                            "is_maker": json["m"].as_bool().unwrap_or(false),
