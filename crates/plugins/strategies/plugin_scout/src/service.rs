...
impl OpportunityLogger {
    ...
    pub fn log(&mut self, opp: &Opportunity) {
            ...
            && self.last_verdict == Some(opp.verdict)
        {
            ...
            tracing::info!("{}", msg);
            *self.data.lock().unwrap() = msg.into_bytes();
            return;
        }
...
impl ScoutService {
    ...
    pub async fn start(&mut self) -> Result<(), reqwest::Error> {
        ...
        self.analysis_task = Some(tokio::spawn(async move {
            Self::analysis_loop(&market, data).await;
        }));
    ...
    async fn analysis_loop(market: &Arc<Mutex<MarketState>>, data: Arc<Mutex<Vec<u8>>>) {
        tracing::info!(
        ...
        let analyzer = OrderbookFluxAnalyzer::new();
        let mut logger = OpportunityLogger::new(data);
