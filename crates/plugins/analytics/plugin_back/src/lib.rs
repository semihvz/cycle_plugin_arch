...
pub fn process_and_persist_tacusdt_data(symbol: &str, interval: &str, bars: &[Bar], db_path: &str) -> CollectorStatus {
    ...
    CollectorStatus {
        ...
        total_trades_detected: detected_trades.len(),
...
async fn fetch_and_run_collector(data_arc: Arc<Mutex<HashMap<String, Value>>>, db_path: String) {
    ...
    let url_15m = "https://fapi.binance.com/fapi/v1/klines?symbol=TACUSDT&interval=15m&limit=1500";
    let resp = client.get(url_15m).send().await;
    ...
    if let Ok(r) = resp {
        if r.status().is_success() {
            if let Ok(raw) = r.json::<Vec<Vec<serde_json::Value>>>().await {
                ...
                if bars.len() > 100 {
                    let status = process_and_persist_tacusdt_data("TACUSDT", "15m", &bars, &db_path);
                    if let Ok(val) = serde_json::to_value(&status) {
                        let mut guard = data_arc.lock().unwrap();
                        guard.insert("tacusdt_collector_status".to_string(), val);
                    }
                }
            }
        }
...
pub unsafe extern "C" fn init_plugin(state_out: *mut *mut c_void) -> unsafe extern "C" fn(*mut c_void, u32, *const u8, usize, *mut u8, usize) -> usize {
    ...
    let db_path = "/home/smhvz/Desktop/cycle-orc/data/tacusdt_collector.db".to_string();

    let data_sync = data.clone();
    let db_path_sync = db_path.clone();
    runtime.block_on(async move {
        fetch_and_run_collector(data_sync, db_path_sync).await;
    });
        ...
        0 => { // Start
            ...
            state.runtime.spawn(async move {
                while is_running_arc.load(Ordering::Relaxed) {
                    fetch_and_run_collector(data_arc.clone(), db_path_clone.clone()).await;
            ...
            for (stream_id, val) in guard.iter() {
                if let Ok(st) = serde_json::from_value::<CollectorStatus>(val.clone()) {
                    report_str.push_str(&format!(
                        "[{}] Symbol: {} | Interval: {} | Toplam Çekilen Bar: {}\n",
                        stream_id, st.symbol, st.interval, st.total_bars_fetched
                    ));
                    report_str.push_str(&format!(
                        "📊 Tespit Edilen İşlem Sayısı: {} adet | Saklanan 100-Bar Mum Kaydı: {} satır\n",
                        st.total_trades_detected, st.total_lookback_bars_persisted
                    ));
                    report_str.push_str(&format!(
