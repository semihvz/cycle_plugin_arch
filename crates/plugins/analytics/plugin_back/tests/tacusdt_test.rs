use plugin_back::{process_and_persist_tacusdt_data, Bar};
use rusqlite::Connection;

#[tokio::test]
async fn test_live_collector_pipeline() {
    ...
    let resp = client.get(url_15m).send().await.unwrap();
    let text = resp.text().await.unwrap();

    let raw: Vec<Vec<serde_json::Value>> = match serde_json::from_str(&text) {
        Ok(v) => v,
        Err(e) => panic!("Failed to parse Binance 15m klines: {} - Body: {}", e, &text[0..text.len().min(200)]),
    };
    ...
    println!("\n==========================================================================================");
    println!("📦 TACUSDT VERİ TOPLAMA VE 100-BAR SQLITE KAYIT EKLENTİSİ CANLI RAPORU");
    println!("==========================================================================================");
    println!("Sembol ve Zaman Dilimi    : {} / {}", status.symbol, status.interval);
    println!("Çekilen Toplam Bar        : {} adet 15m mum", status.total_bars_fetched);
    ...
    // Verify SQLite tables using rusqlite connection
