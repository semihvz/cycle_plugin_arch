        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_liquidations(symbols, is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_liquidations(
    ...
    let ws_url = if symbols.is_empty() || symbols.iter().any(|s| s == "ALL") {
        ...
        let streams: Vec<String> = symbols.iter().map(|s| format!("{}@forceOrder", s.to_lowercase())).collect();
                            ...
                            Some(Ok(Message::Text(text))) => {
                                ...
                                if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                    ...
                                    if let Some(o) = json.get("o") {
                                        ...
                                        let price = o["p"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0);
                                        let qty = o["q"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0);
                                        let total_usdt = price * qty;

                                        let output = serde_json::json!({
                                            "symbol": symbol,
                                            "side": o["S"].as_str().unwrap_or(""),
                                            "order_type": o["o"].as_str().unwrap_or(""),
                                            "price": price,
                                            "qty": qty,
                                        ...
                                        let mut arr: Vec<serde_json::Value> = serde_json::from_slice(&guard).unwrap_or_default();
                                        arr.push(output);
                                        if arr.len() > 50 {
                                            arr.remove(0);
                                        }
                                        *guard = serde_json::to_vec_pretty(&arr).unwrap_or_default();
                                    }
                                }
                            }
