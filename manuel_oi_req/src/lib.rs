...
pub unsafe extern "C" fn init_plugin(state_out: *mut *mut c_void) -> unsafe extern "C" fn(*mut c_void, u32, *const u8, usize, *mut u8, usize) -> usize {
    let state = Box::new(PluginState {
        ...
        data: Arc::new(Mutex::new(b"Manuel OI request eklentisi hazir.".to_vec())),
...
unsafe extern "C" fn handle_endpoint(
    ...
    match endpoint_id {
        0 => { // Start
            ...
            *guard = b"Manuel OI request eklentisi baslatildi. TUI uzerinden 'o' tusuna basarak form doldurun.".to_vec();
            0
        }
        1 => { // Stop
            ...
            if payload_len > 0 {
                ...
                if let Ok(msg) = serde_json::from_slice::<serde_json::Value>(slice) {
                    if msg["action"].as_str() == Some("manual_trigger") {
                        ...
                        let interval = msg["interval"].as_str().unwrap_or("5m");
                        let limit = msg["limit"].as_i64().unwrap_or(30);
                        
                        let request_msg = serde_json::json!({
                            "to": "plugin_oi_fetcher",
                            "from": "plugin_manuel_oi_req",
                            "action": "fetch_oi",
                            "symbol": symbol,
                            "interval": interval,
                            "limit": limit
                        ...
                        let mut guard = state.data.lock().unwrap();
                        *guard = format!("Istek plugin_oi_fetcher'a iletildi ({} {} {} veri). Yanit bekleniyor...", symbol, interval, limit).into_bytes();
                    } else if msg["action"].as_str() == Some("fetch_oi_response") {
                        let mut guard = state.data.lock().unwrap();
