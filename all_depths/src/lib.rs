        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_combined_depths(is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_combined_depths(
    is_running: Arc<AtomicBool>,
    data: Arc<Mutex<Vec<u8>>>,
    ...
    // Combined stream for depth20@100ms
    let url = "wss://fstream.binance.com/stream?streams=btcusdt@depth20@100ms/ethusdt@depth20@100ms/aceusdt@depth20@100ms";
                                ...
                                Some(Ok(Message::Text(text))) => {
                                    ...
                                    if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                        if let Some(json) = wrapper.get("data") {
                                            let stream_name = wrapper["stream"].as_str().unwrap_or("");
                                            let symbol = if stream_name.starts_with("btcusdt") {
                                                "BTCUSDT"
                                            } else if stream_name.starts_with("ethusdt") {
                                                "ETHUSDT"
                                            } else if stream_name.starts_with("aceusdt") {
                                                "ACEUSDT"
                                            } else {
                                                "UNKNOWN"
                                            };
                                            ...
                                            latest_data.insert(symbol.to_string(), output.clone());
