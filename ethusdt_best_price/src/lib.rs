...
async fn stream_book_ticker(
    ...
    let url = "wss://fstream.binance.com/ws/ethusdt@bookTicker";
    ...
    std::thread::spawn(move || {
        if let Ok(conn) = rusqlite::Connection::open("ETHUSDT_data.db") {
            ...
            if let Ok(mut stmt) = conn.prepare(
                "INSERT INTO bookticker (symbol, best_bid, best_bid_qty, best_ask, best_ask_qty, spread, timestamp, event_time, local_recv_time_ms)
