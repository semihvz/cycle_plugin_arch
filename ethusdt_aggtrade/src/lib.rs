// ═══════════════════════════════════════════════════════════════════
// Plugin: ethusdt_best_price
// Binance Futures BookTicker Stream — ETHUSDT
...
async fn stream_book_ticker(
    ...
    let url = "wss://fstream.binance.com/ws/ethusdt@aggTrade";
    ...
    std::thread::spawn(move || {
        if let Ok(conn) = rusqlite::Connection::open("ETHUSDT_data.db") {
            let _ = conn.execute(
            ...
            if let Ok(mut stmt) = conn.prepare(
                "INSERT INTO aggtrade (symbol, price, quantity, is_buyer_maker, timestamp, event_time, local_recv_time_ms)
                ...
                while let Some(record) = db_rx.blocking_recv() {
                    ...
                    let price: f64 = record["price"].as_str().unwrap_or("0").parse().unwrap_or(0.0);
                    let quantity: f64 = record["quantity"].as_str().unwrap_or("0").parse().unwrap_or(0.0);
                    let is_buyer_maker = record["is_buyer_maker"].as_bool().unwrap_or(false) as i32;
                            ...
                            match msg {
                                Some(Ok(Message::Text(text))) => {
                                    ...
                                    if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                                        let price = json["p"].as_str().unwrap_or("0");
                                        let quantity = json["q"].as_str().unwrap_or("0");
                                        let is_buyer_maker = json["m"].as_bool().unwrap_or(false);
