...
pub unsafe extern "C" fn init_plugin(
    ...
    let state = Box::new(PluginState {
        ...
        data: Arc::new(Mutex::new(b"Tum piyasa likidasyonlari bekleniyor... (!forceOrder@arr)".to_vec())),
        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_all_liquidations(is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_all_liquidations(
    is_running: Arc<AtomicBool>,
    data: Arc<Mutex<Vec<u8>>>,
