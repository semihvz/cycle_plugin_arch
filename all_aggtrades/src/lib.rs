        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_combined_aggtrades(is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_combined_aggtrades(
    is_running: Arc<AtomicBool>,
    data: Arc<Mutex<Vec<u8>>>,
    ...
    // Binance Combined stream for trades
    let url = "wss://fstream.binance.com/stream?streams=btcusdt@trade/ethusdt@trade/aceusdt@trade";
                                ...
                                Some(Ok(Message::Text(text))) => {
                                    ...
                                    if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                        if let Some(json) = wrapper.get("data") {
                                            ...
                                            let qty = json["q"].as_str().unwrap_or("0");
