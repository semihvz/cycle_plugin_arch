...
from run_parquet_ms_analyzer_backtest import backtest_parquet_symbol, PARQUET_DIR

EXCEL_OUT_DIR = "/home/smhvz/Desktop/cycle-orc/data/symbol_trade_logs_excel"
os.makedirs(EXCEL_OUT_DIR, exist_ok=True)

def process_and_save_symbol_excel(symbol):
    res = backtest_parquet_symbol(symbol, position_size_usdt=100.0, initial_capital_usdt=100.0)
...
def main():
    ...
    print("==========================================================================================")
    print(f"📊 330 PARİTENİN TÜM İŞLEM KAYITLARI EXCEL DOSYALARINA AKTARILIYOR")
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = {executor.submit(process_and_save_symbol_excel, sym): sym for sym in symbols}
            ...
            try:
                ...
                if res:
                    ...
                    if saved_count % 30 == 0 or saved_count == 1:
                        print(f"[{saved_count:3d}/330] ✅ {s_name:<14} -> {t_count:<4} işlem kaydedildi: {os.path.basename(out_f)}")
    ...
    if all_dfs:
        ...
        master_file = "/home/smhvz/Desktop/cycle-orc/all_330_symbols_master_trade_logs.xlsx"
        master_df.to_excel(master_file, index=False)
        print(f"📂 TÜM İŞLEMLER BİRLEŞİK MASTER EXCEL DOSYASI: {master_file}")
    print("==========================================================================================")
