...
def fetch_klines(symbol="TACUSDT", interval="15m", limit=1500):
...
def main():
    ...
    fixed_pos_size = 50.0
    lookback = 100
    db_path = "/home/smhvz/Desktop/cycle-orc/tacusdt_15m_backtest.db"

    print("==========================================================================================")
    print(f"🔥 TACUSDT {interval} ZAMAN DİLİMİ İÇİN BACKTEST VE MAKİNE ÖĞRENMESİ PİPELİNE")
    print("==========================================================================================")
    ...
    print(f"\n📊 TACUSDT {interval} Backtest Tamamlandı:")
    ...
    # Machine Learning Model Training on 15m
    feature_cols = [
    ...
    print("------------------------------------------------------------------------------------------")
    print(f"🤖 MAKİNE ÖĞRENMESİ (15m MODELİ) EĞİTİMİ VE FİLTRE PERFORMANSI:")
    print("------------------------------------------------------------------------------------------")
    ...
    print(f"{'Ham Strateji (15m)':<18} | {len(df):<12} | %{(y.mean()*100):<13.2f} | {base_pnl:<+15.2f} | {base_pf:<13.2f}")
    ...
    # Fit final model and save 15m joblib files
    rf.fit(X_scaled, y)
    models_dir = "/home/smhvz/Desktop/cycle-orc/ml_model_suite/models"
    os.makedirs(models_dir, exist_ok=True)
    joblib.dump(rf, os.path.join(models_dir, "tacusdt_15m_ml_model.joblib"))
    joblib.dump(scaler, os.path.join(models_dir, "scaler_15m.joblib"))
    
    # Save CSV export for 15m
    df.to_csv("/home/smhvz/Desktop/cycle-orc/tacusdt_15m_closed_trades.csv", index=False)
    print("✅ 15m Modeli ve Veri Seti Başarıyla Kaydedildi: 'tacusdt_15m_ml_model.joblib'")

    # Live Inference on latest 15m Binance Futures Candles
    print("\n==========================================================================================")
    print("⚡ TACUSDT 15m CANLI PİYASA İNFERENCE (TAHMİN) TESTİ:")
    print("==========================================================================================")
    bars_live = fetch_klines(symbol, "15m", 100)
    ...
    print(f"📊 Sembol: TACUSDT 15m | Canlı Fiyat: {entry_p} USDT")
    print(f"🎯 Yapay Zeka 15m Kazanma Olasılığı: %{live_win_prob*100:.2f}")
