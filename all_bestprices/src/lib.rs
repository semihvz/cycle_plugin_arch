        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_combined_bestprices(is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_combined_bestprices(
    is_running: Arc<AtomicBool>,
    data: Arc<Mutex<Vec<u8>>>,
    ...
    // Combined stream for bookTicker
    let url = "wss://fstream.binance.com/stream?streams=btcusdt@bookTicker/ethusdt@bookTicker/aceusdt@bookTicker";
                                ...
                                Some(Ok(Message::Text(text))) => {
                                    ...
                                    if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                        if let Some(json) = wrapper.get("data") {
                                            ...
                                            let best_bid = json["b"].as_str().unwrap_or("0");
                                            let best_bid_qty = json["B"].as_str().unwrap_or("0");
                                            let best_ask = json["a"].as_str().unwrap_or("0");
                                            let best_ask_qty = json["A"].as_str().unwrap_or("0");
                                            let timestamp = json["T"].as_i64().unwrap_or(0);
                                            let event_time = json["E"].as_i64().unwrap_or(0);

                                            let best_bid_f: f64 = best_bid.parse().unwrap_or(0.0);
                                            let best_ask_f: f64 = best_ask.parse().unwrap_or(0.0);
                                            let spread = if best_ask_f > 0.0 { best_ask_f - best_bid_f } else { 0.0 };
