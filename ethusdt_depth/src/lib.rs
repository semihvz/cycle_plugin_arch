...
async fn stream_book_ticker(
    ...
    let url = "wss://fstream.binance.com/ws/ethusdt@depth20@100ms";
    ...
    std::thread::spawn(move || {
        if let Ok(conn) = rusqlite::Connection::open("ETHUSDT_data.db") {
            ...
            if let Ok(mut stmt) = conn.prepare(
                "INSERT INTO depth (symbol, bids, asks, timestamp, event_time, local_recv_time_ms)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6)"
            ) {
                while let Some(record) = db_rx.blocking_recv() {
                    ...
                    let bids = record["bids"].to_string();
                    let asks = record["asks"].to_string();
                            ...
                            match msg {
                                Some(Ok(Message::Text(text))) => {
                                    ...
                                    if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                                        let bid = json["b"][0][0].as_str().unwrap_or("0");
                                        let ask = json["a"][0][0].as_str().unwrap_or("0");
                                        let bid_qty = json["b"][0][1].as_str().unwrap_or("0");
                                        let ask_qty = json["a"][0][1].as_str().unwrap_or("0");
