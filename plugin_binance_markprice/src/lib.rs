        ...
        0 => { // Start
            ...
            *state.shutdown_tx.lock().unwrap() = Some(tx);
            ...
            state.runtime.spawn(async move {
                stream_markprice(symbols, is_running, data, rx).await;
            });
            0
        }
        1 => { // Stop
...
async fn stream_markprice(
    ...
    let ws_url = if symbols.is_empty() || symbols.iter().any(|s| s == "ALL") {
        ...
        let streams: Vec<String> = symbols.iter().map(|s| format!("{}@markPrice@1s", s.to_lowercase())).collect();
                        ...
                        match msg {
                            Some(Ok(Message::Text(text))) => {
                                ...
                                if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                                    let json_arr = if wrapper.is_array() {
                                        wrapper.as_array().cloned().unwrap_or_default()
                                    } else if let Some(arr) = wrapper.get("data").and_then(|d| d.as_array()) {
                                        arr.clone()
                                    } else if let Some(obj) = wrapper.get("data") {
                                        vec![obj.clone()]
                                    } else {
                                        vec![wrapper.clone()]
                                    };
                                    ...
                                    for item in json_arr {
                                        ...
                                        if !symbol.is_empty() {
                                            let output = serde_json::json!({
                                                "symbol": symbol,
                                                "mark_price": item["p"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0),
                                                "index_price": item["i"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0),
                                                "funding_rate": item["r"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0),
                                                "next_funding_time": item["T"].as_i64().unwrap_or(0),
                                                "event_time": item["E"].as_i64().unwrap_or(0),
